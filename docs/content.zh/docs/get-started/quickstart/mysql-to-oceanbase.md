...
# Streaming ELT 同步 MySQL 到  OceanBase

这篇教程将展示如何基于 Flink CDC 快速构建 MySQL 到 OceanBase 的 Streaming ELT 作业，包含整库同步、表结构变更同步和分库分表同步的功能。
...
1. 下载 [Flink 1.18.1](https://archive.apache.org/dist/flink/flink-1.18.1/flink-1.18.1-bin-scala_2.12.tgz)，解压后得到 flink-1.18.1 目录。
   使用下面的命令跳转至 Flink 目录下，并且设置 FLINK_HOME 为 flink-1.18.1 所在目录。
...
- OceanBase: 存储从 MySQL 中根据规则映射过来的结果表
...
2. 下载下面列出的 connector 包，并且移动到 `lib` 目录下；
   **下载链接只对已发布的版本有效, SNAPSHOT 版本需要本地基于 master 或 release- 分支编译.**
   ...
   - [Apache OceanBase pipeline connector 3.2.0](https://search.maven.org/remotecontent?filepath=org/apache/flink/flink-cdc-pipeline-connector-oceanbase/3.2.0/flink-cdc-pipeline-connector-oceanbase-3.2.0.jar)
...
3.编写任务配置 yaml 文件。
下面给出了一个整库同步的示例文件 `mysql-to-oceanbase.yaml`：
...
提交成功后，返回信息如：
   ...
   ```
在 Flink Web UI，可以看到一个名为 `Sync MySQL Database to OceanBase` 的任务正在运行。 

{{< img src="/fig/mysql-oceanbase-tutorial/mysql-to-oceanbase.png" alt="MySQL-to-OceanBase" >}}

通过数据库连接工具例如 Dbeaver 等连接到 jdbc:mysql://127.0.0.1:2881， 可以查看 OceanBase 中写入了三张表的数据。
...
接下来，修改 MySQL 数据库中表的数据，OceanBase 中显示的订单数据也将实时更新：
...
本教程结束后，在 `docker-compose.yml` 文件所在的目录下执行如下命令停止所有容器：
   ...
   docker-compose down
   ```
在 Flink 所在目录 `flink-1.18.1` 下执行如下命令停止 Flink 集群：
